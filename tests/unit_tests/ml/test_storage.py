...
class TestDiskStorage:
    ...
    def test_load_many_from_folder(self, pickle_with_x_model, pickle_with_x_model_two):
        ...
        # Order in folder
        assert str(models_bin[0].path) == str(pickle_with_x_model)
        assert models_bin[0].raw_data == with_x_model_data
        assert str(models_bin[1].path) == str(pickle_with_x_model_two)
        assert models_bin[1].raw_data == with_x_model_data_two
